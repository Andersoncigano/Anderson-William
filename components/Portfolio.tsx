      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {PORTFOLIO_ITEMS.map((item) => {
            ...
            const CardContent = () => (
              <>
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-5">
                  ...
                  <h4 className="text-sm font-bold uppercase transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 delay-100 flex items-center justify-between">
                    {item.title}
                    {item.link && <ExternalLink size={14} className="ml-2 text-white/70" />}
                  </h4>
                </div>
              </>
            );
            ...
            // Estilos comuns
            const commonClasses = "group relative aspect-square overflow-hidden bg-gray-900 cursor-pointer block";

            // Renderiza <a> se tiver link, ou <div> se não tiver
            return item.link ? (
              <a 
                key={item.id} 
                href={item.link}
